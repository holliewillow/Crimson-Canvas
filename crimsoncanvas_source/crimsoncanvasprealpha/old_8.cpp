...
namespace Grid
{
	...
	void get_Player_Tile(Blood_Grid *b, Player::Player *p, int i, int j)
	{
		p->hurt_boxes[i][j].current_row = ((int)p->hurt_boxes[i][j].y + p->hurt_boxes[i][j].h / 3) / b->tile_size;
		p->hurt_boxes[i][j].current_column = ((int)p->hurt_boxes[i][j].x + p->hurt_boxes[i][j].w / 3) / b->tile_size;
	}

	void update(Blood_Grid *b, Player::Player *p)
	{
		Player::check_Hit(p);
		for (int i = 0; i < p->n_players; i++)
		{
			if (p->got_hurt[i] == 1)
			{
				for (int j = 0; j < Character_Data::n_hurt_boxes; j++)
				{
					if (p->hurt_boxes[i][j].was_hit == 1)
					{
						get_Player_Tile(b, p, i, j);

						for (int h = -1; h <= 1; h++)
						{
							for (int l = -1; l <= 1; l++)
							{
								b->grid[p->hurt_boxes[i][j].current_row][p->hurt_boxes[i][j].current_column] = i;
							}
						}
						p->hurt_boxes[i][j].was_hit = 0;
					}
				}
				p->got_hurt[i] = 0;
			}
