...
namespace Player
{
	...
	void player_Character_Setup(Player *p, Character_Data::TK *tk, SDL_Texture *TK_texture, Character_Data::Camila *camila, SDL_Texture *Camila_texture, Character_Data::Wrecca *wrecca, int* chosen_character, int ground_y)
	{
...
int main(int argc, char **argv)
{
	...
	for (;;)
	{
		...
		else if (game_state == local_multi)
		{
			...
			Player::player_Character_Setup(&p, &tk, TK_texture, &camila, Camila_texture, &wrecca, chosen_character, ground_y);
			...
			for (;;)
			{
				...
				current_time = SDL_GetTicks();
				if (1000.0 / (current_time - last_frame_time) <= 60)
				{
					last_frame_time = current_time;
					/*
					front punch is x
					...
					*/
					...
					if (p.state[1] == Player::jumping)
					{
						...
						{
							p.current_frame[1] = (p.current_frame[1] + 1) % (p.n_jump_frames[1] + 1);
